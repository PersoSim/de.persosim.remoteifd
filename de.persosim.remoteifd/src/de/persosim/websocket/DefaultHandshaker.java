...
public class DefaultHandshaker implements TlsHandshaker
{
		...
		try {
			...
			protocol.accept(new DefaultTlsServer(crypto) {
				...
				public void notifyOfferedCipherSuites(int[] arg0) throws IOException
				{
					...
					BasicLogger.log(cipherSuites, LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.REMOTE_IFD_ID));
				}
			...
			BasicLogger.logException("Other side closed the connection", e, LogLevel.WARN, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.REMOTE_IFD_ID));
		}
		return false;
	}
	...
	{
		try {
			BasicLogger.log("Closing TLS connection", LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.REMOTE_IFD_ID));
			protocol.close();
		}
		catch (IOException e) {
			// Expected when peer closes socket too early
			BasicLogger.logException("Exception during closing of tls server, probably due to early close", e, LogLevel.INFO, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.REMOTE_IFD_ID));
		}
	}
